...
export const fetchBpoom = (uuid, queryParams = {}) =>
  api(`/birth_app/bpooms/${uuid}`, (bpoom, dispatch) =>
    dispatch({ type: BPOOM, bpoom: overrideBpoom(bpoom, queryParams) }),
  )

export const saveMsg = (uuid, guest_book_msg) =>
  api(
    { path: `/birth_app/guest_book_msgs/${uuid}`, method: 'POST', data: { guest_book_msg } },
    (guestBookMsg, dispatch) => dispatch({ type: GUEST_BOOK_MSG_SAVE, guestBookMsg }),
  )

export const deleteMsg = (id, uuid) =>
  api({ path: `/birth_app/guest_book_msgs/${id}/${uuid}`, method: 'DELETE' }, (json, dispatch) =>
    dispatch({ type: GUEST_BOOK_MSG_DELETE, id: json.id }),
  )

export const saveMangopayAccount = mangopay_account =>
  api({ path: `/birth_app/mangopay_accounts`, method: 'POST', data: { mangopay_account } })

export const saveMangopayPayment = mangopay_payment =>
  api({ path: `/birth_app/mangopay_payments`, method: 'POST', data: { mangopay_payment } })
