...
export default class extends Component {
  constructor(props) {
    ...
    let content = getComputedStyle(this.root).getPropertyValue('content')
    this.specialCssVariables = content ? content.slice(1, -1).split(' ') : []
  }
    ...
    this.specialCssVariables.forEach(varName => {
      ...
      if (match) {
        ...
        if (variables[parentVar]) {
          variables[varName] = Color[funcName](
            variables[parentVar],
            'rgba' === funcName ? value / 100 : +value,
          )
        }
      }
    })
