...
const DEMO_BPOOM = {
  // TODO: translate
  photo_urls: createPhotoURLs(imgPath('/avatars/baby.svg')),
  baby_name: 'Demo',
  parent_1_name: 'Maman',
  parent_2_name: 'Papa',
  size: 'XXX',
  ...
  hair_color: 'XXX',
  eyes_color: 'XXX',
  parent_1_photo_urls: createPhotoURLs(imgPath('/avatars/parent-1.svg')),
  parent_2_photo_urls: createPhotoURLs(imgPath('/avatars/parent-2.svg')),
...
export default function overrideBpoom(bpoom, params, canEdit) {
  if (params.aha || (params.queryParams || {}).aha) {
    Object.keys(DEMO_BPOOM).forEach((prop) => {
      if (!bpoom[prop] || !bpoom[prop].length) bpoom[prop] = DEMO_BPOOM[prop]
    })
  }
