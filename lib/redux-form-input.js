...
export function InputField({ hint, input, label, type, meta: { touched, error }={}, ...inputProps }) {
  ...
  return (
    <FormGroup color={status}>
      ...
      <Input type={type || 'text'} id={htmlId} autoComplete="off" {...inputProps} />
...
export function SelectField({ hint, input, label, options, i18n, includeBlank, meta: { touched, error }={}, ...inputProps }) {
  inputProps = { ...input, ...inputProps };
  let feedback = touched && error;
  let status   = touched && error ? 'danger' :  '';
  let htmlId   = sanitizeId(inputProps.id || inputProps.name);
  ...
  return (
    <FormGroup color={status}>
      <Label className="form-control-label" for={htmlId}>{label}</Label>
      <Input type="select" id={htmlId} autoComplete="off" {...inputProps}>
        { includeBlank ? <option key={0} value="">{'string' === typeof includeBlank ? includeBlank : ''}</option> : '' }
        { (options || []).map((opt, index) => {
            ...
            return i18n && 'false' !== i18n
              ...
              : <option key={index + 1} value={opt[1]}>{opt[0]}</option>
          }) }
      </Input>
      {feedback ? <FormFeedback>{feedback}</FormFeedback> : ''}
      {hint ? <FormText color="muted">{hint}</FormText> : ''}
  ...
  return (
    <FormGroup color={status}>
      <Label check className="form-control-label" for={htmlId}>
        <Input type={type} id={htmlId} autoComplete="off" {...inputProps} />
        {` ${label}`}
      </Label>
      {feedback ? <FormFeedback>{feedback}</FormFeedback> : ''}
      {hint ? <FormText color="muted">{hint}</FormText> : ''}
